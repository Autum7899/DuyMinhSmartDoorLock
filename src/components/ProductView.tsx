...
export default function ProductView({ product }: { product: ProductViewProps }) {
    ...
    const renderFeatures = () => {
        ...
        return (
            // OPTIMIZED: Added a bottom margin to this block for better separation
    ...
    return (
        <>
            {/* OPTIMIZED: Changed to py-* to add padding on top as well, giving the whole section more space */}
            <div className="bg-gray-50 py-8 md:py-12">
                <div className="container mx-auto px-4">
                    {/* The padding p-6 md:p-8 here is good, no change needed */}
                    <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                            ...
                            {/* Cột phải: Thông tin */}
                            {/* OPTIMIZED: Removed space-y-6 to apply margins manually for more precise control */}
                            <div className="flex flex-col">
                                {/* OPTIMIZED: Added specific bottom margin for better spacing control */}
                                <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-4">{product.name}</h1>
                                
                                {/* OPTIMIZED: Grouped Price and Status, added a larger bottom margin to separate this info block */}
                                <div className="mb-6">
                                    <span className="text-3xl md:text-4xl font-bold text-red-600 block mb-2">{formatPrice(Number(product.price_retail_with_install))}</span>
                                ...
                                {/* OPTIMIZED: This is the main action block. Removed space-y-4 for manual control. */}
                                <div>
                                    {product.quantity > 0 && (
                                        // OPTIMIZED: Added a bottom margin to separate the quantity selector from the buttons
                                    ...
                                    {/* OPTIMIZED: Added a larger bottom margin (mb-6) to strongly separate the main CTA buttons from the secondary contact buttons */}
                                    ...
                                    {/* OPTIMIZED: Increased gap from gap-2 to gap-3 for more breathing room between contact buttons */}
